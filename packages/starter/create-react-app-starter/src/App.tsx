...
import {
    ...
    Transaction,
    LAMPORTS_PER_SOL,
    TransactionInstruction, SystemProgram, TransferParams,
} from '@solana/web3.js';
...
//        const programId = new PublicKey('DVbC3AGo1MXCTqVGRL9kANVdG2RsyRb9dxBWK77FY14X'); -- devnet
        const programId = new PublicKey('J9qDbjQBHEFY5fPB3fXS8je7UFeS2xW5jrzDVwbXzQUV'); //-- localnet
        ...
        const buffer = Buffer.alloc(1 + 8); // 1 byte for the instruction index, 8 bytes for u64
        buffer[0] = 2; // the index of the 'transfer' instruction
        buffer.writeBigInt64LE(BigInt(amount * LAMPORTS_PER_SOL), 1); // the amount to transfer, written as a 64-bit integer
...
const sendSystemTransfer = async (wallet : WalletContextState, connection: Connection, targetPublicKey: string, amount: number) => {
    try {
        const publicKey = wallet.publicKey!;
        console.log("PublicKey: ", publicKey.toBase58());
        console.log("Connection", connection);
        let lamportBigInt = BigInt(amount * LAMPORTS_PER_SOL);
        const transferParams : TransferParams = {
            fromPubkey: publicKey,
            toPubkey: new PublicKey(targetPublicKey),
            lamports: lamportBigInt
        };
        let transaction = new Transaction();
        transaction.add(SystemProgram.transfer(transferParams));
        transaction.recentBlockhash = (await connection.getLatestBlockhash()).blockhash;
        transaction.feePayer = publicKey;
...
const Content: FC = () => {
    ...
    return (
        <div className="App">
            <input className="wallet-adapter-input" type="text" value={targetPublicKey} onChange={handlePublicKeyChange}
                   placeholder="Public Key" style={{ width: '20%' }} />
            <input className="wallet-adapter-input" type="number" value={amount} onChange={handleAmountChange}
            ...
            <button className="wallet-adapter-button wallet-adapter-button-trigger"
                    onClick={() => {
                        if (wallet.publicKey && wallet.signTransaction) {
                            sendSystemTransfer(wallet, connection, targetPublicKey, Number.parseFloat(amount))
                        } else {
                            console.log("publicKey: ", publicKey);
                            console.log("signTransaction: ", signTransaction);
                            console.log("Connection: ", connection);
